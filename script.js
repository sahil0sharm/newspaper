...
document.querySelector("#international").addEventListener("click", function (e) {
    fetch("https://newsapi.org/v2/top-headlines&apiKey=dc61cf5d1b6e4d6e9042a01d34868313")
        .then(responses => responses.json())
        .then((news) => {
            for (let i = 0; i < news.articles.length - 1; i++) {
                newshtml += `<div class="ambani">
...
document.querySelector("#technology").addEventListener("click", function (e) {
    fetch("https://newsapi.org/v2/top-headlines?category=technology&apiKey=dc61cf5d1b6e4d6e9042a01d34868313")
        .then(responses => responses.json())
        .then((news) => {
            for (let i = 0; i < news.articles.length - 1; i++) {
                newshtml += `<div class="ambani">
